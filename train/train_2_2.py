...
def test_epoch(epoch, test_loader, model, criterion):
	...
	os.makedirs("confusion_matrix_2_2", exist_ok="True")
	model.eval()
	...
	df_cm = pd.DataFrame(np.array(confusion_matrix(target_list, predicted_list)), index = [i for i in ['CAS', 'CAS & DAS', 'DAS', 'Normal','Poor Quality']],
                  columns = [i for i in ['CAS', 'CAS & DAS', 'DAS', 'Normal','Poor Quality']])
	...
	plt.savefig(os.path.join("confusion_matrix_2_2", str(epoch)+".png"))
...
def training_testing(train_loader, test_loader, model, criterion, model_opt, resume):
	...
	epoch = 0
	checkpoint_dir = "/ssd_scratch/cvit/seshadri_c/temp_task_2_2/"
	os.makedirs(checkpoint_dir, exist_ok=True)

	checkpoint_path = checkpoint_dir + "checkpoint_2_2.pt"
	if resume:
		model, model_opt, epoch = load_ckp(checkpoint_path, model, model_opt)
		resume = False
		print("Resuming Training from Epoch Number : ", epoch)
	checkpoint_duplicate_dir = "/scratch/seshadri_c/temp_task_2_2/"
